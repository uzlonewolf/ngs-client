...
class handle_packet:
    ...
    def handle_packet( self, pkt ):
        ...
        if( pkt.is_server_mode ):
            ...
            if( self.gamedata.last_game_state != pkt.data.mode ):
                print '!! Game Mode Changed !!'

            if( (pkt.data.mode == SERVER_MODE_IDLE) or (pkt.data.mode == SERVER_MODE_RUNNING) ):
                self.gamedata.game_length = 0
            elif( pkt.data.mode == SERVER_MODE_GAME_START ):
                if( (self.gamedata.last_game_state != SERVER_MODE_GAME_START) or (self.gamedata.game_length < 1) ):
                    self.gamedata.game_length = pkt.data.time_remaining

                if( self.gamedata.game_length > 0 ):
                    print 'game progress:', int((float(pkt.data.time_remaining) / self.gamedata.game_length) * 100), '%'

            self.gamedata.last_game_state = pkt.data.mode
